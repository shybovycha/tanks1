#include "WorkingWithOgre.h"

Workingwithogre::Workingwithogre(void)
    : mRoot(0),
    mCamera(0),
    mSceneMgr(0),
    mWindow(0),
...
bool Workingwithogre::go(void) {
    ...
    std::cerr << "--- Hello, Background!\n";

    //ЗАГРУЖАЕМ МОДЕЛИ
    //====================================================
    Ogre::ResourceGroupManager::getSingleton().initialiseAllResourceGroups();

    std::cerr << "--- Hello, Resources!\n";

    //ПОЛ
    //====================================================
    Ogre::Plane plane(Ogre::Vector3::UNIT_Y, 0);
    Ogre::MeshManager::getSingleton().createPlane("ground", Ogre::ResourceGroupManager::
        DEFAULT_RESOURCE_GROUP_NAME, plane, 1500, 1500, 20, 20, true, 1, 32, 32,
        Ogre::Vector3::UNIT_Z);
    ...
    //Tank1
    //====================================================
    Ogre::Entity* tank1Entity = mSceneMgr->createEntity("tank1", "Body.mesh");
    ...
    Ogre::Bone* tower1Bone = tower1Skeleton->getBone("Bone");

    Ogre::Matrix3 tower1Orientation;
    tower1Orientation.FromEulerAnglesXYZ(Ogre::Degree(90), Ogre::Degree(-90), Ogre::Degree(0));
    Ogre::TagPoint* tower1 = tank1Entity->attachObjectToBone("TowerBone", tower1Entity, tower1Orientation);
    ...
    Ogre::Vector3 gun1Offset(0.85, 0, 0);
    Ogre::TagPoint* gun1 = tower1Entity->attachObjectToBone("Bone", gun1Entity, gun1Orientation, gun1Offset);
    gun1->scale(0.1, 0.1, 0.1);

    mSkeletonDebugger = new SkeletonDebug(tank1Entity, mSceneMgr, mCamera, 1.0);
...
bool Workingwithogre::frameRenderingQueued(const Ogre::FrameEvent& evt) {
...
//====================================================
bool Workingwithogre::keyPressed( const OIS::KeyEvent &arg ) {
    ...
    mCameraMan->injectKeyDown(arg); return true;
}
bool Workingwithogre::keyReleased( const OIS::KeyEvent &arg ) {
    mCameraMan->injectKeyUp(arg); return true;
}
bool Workingwithogre::mouseMoved( const OIS::MouseEvent &arg ) {
    if (mTrayMgr->injectMouseMove(arg)) return true;
    mCameraMan->injectMouseMove(arg);
    return true;
}
bool Workingwithogre::mousePressed( const OIS::MouseEvent &arg,
    OIS::MouseButtonID id ) {
        if (mTrayMgr->injectMouseDown(arg, id)) return true;
        mCameraMan->injectMouseDown(arg, id); return true;
}
bool Workingwithogre::mouseReleased( const OIS::MouseEvent &arg,
    OIS::MouseButtonID id ) {
        if (mTrayMgr->injectMouseUp(arg, id)) return true;
        mCameraMan->injectMouseUp(arg, id); return true;
}
void Workingwithogre::windowResized(Ogre::RenderWindow* rw) {
...
void Workingwithogre::windowClosed(Ogre::RenderWindow* rw) {
    if( rw == mWindow ) {
        if( mInputManager ) {
